...
char *dirName[]={(char*)"C:",(char*)"D:",(char*)"E:",(char*)"F:"};	
struct inode
{
	char filename[15];
...
int noFiles=0;
struct superblock S[maxDrives];

void inodeinfo(struct inode);
void superblockinfo(struct superblock);
void mount(char **);
char *read_line(void);
int getVal(char *);
	...
	while(1)
	{
		...
		switch (option)
		{
			case mkfs:
				...
				strcpy(S[noFiles].Drive,name);
				S[noFiles].bs = atoi(block);
				S[noFiles].ts = 1000000 * atoi(size);
				S[noFiles].free = S[noFiles].ts / S[noFiles].bs;
				S[noFiles].occupied = 0;
				S[noFiles].openfile = 0;
				S[noFiles].start_offset = sizeof(S[noFiles]);
				superblockinfo(S[noFiles]);
				noFiles++;
				break;
			case cp:
				spacetokenizer(line,parsed);
...
void superblockinfo(struct superblock s)
{
		int i;
...
{
	if(S[driveIndex].openfile < maxNoFiles-1 ){
		struct inode temp;
		...
		printf("Max No of File Limit on Disk Exceeded %d",maxNoFiles);
...
void store(int driveIndex)
{
	...
	if(driveIndex==0)
	{
		fwrite(&noFiles,sizeof(noFiles),1,fd);
	}	
...
void initialize(char *drive,int driveIndex)
{
	...
	if(driveIndex==0){
		fread(&noFiles,sizeof(noFiles),1,fd);
	}
	printf("\nBlock Size: %d\n",S[driveIndex].bs);
	printf("\nTotal Size: %d\n",S[driveIndex].ts);
	printf("\nFree: %d\n",S[driveIndex].free);
